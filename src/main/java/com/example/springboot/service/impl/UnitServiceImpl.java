...
public class UnitServiceImpl extends ServiceImpl<UnitMapper, Unit> implements IUnitService {
    ...
    public Account login(Account account) {
        Unit one = getOne(Wrappers.<Unit>lambdaQuery().eq(Unit::getUsername, account.getUsername()).eq(Unit::getPassword, account.getPassword()));
        if (one != null) {
    ...
    public void register(Account account) {
        Unit one = getOne(Wrappers.<Unit>lambdaQuery().eq(Unit::getUsername, account.getUsername()));
        if (one == null) {
            one = new Unit();
